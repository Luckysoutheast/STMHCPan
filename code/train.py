...
train_file = '../data/train_set/NetMHCpan4.1/train_data_fold0.csv'
val_file = '../data/train_set/NetMHCpan4.1/val_data_fold0.csv'
dataset.load_data(train_file, val_file=val_file)


logging.info("vocab {}".format(dataset.vocab))

model = STSeqCls((23, 100), 2, hidden_size=100, num_layers=3, num_head=5, max_len=50,cls_hidden_size=600,dropout=0.1)
...
class Trainer():
    ...
    def _train(self, epoch):
        ...
        for batch_data in self.train_data:
            ...
            y_score.extend(F.softmax(batch_outputs,dim=1)[:,1].detach().cpu().numpy().tolist())
        ...
        score, f1, auc = get_score(y_true, y_pred, y_score)
